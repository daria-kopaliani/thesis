...
\end{matrix}
\right)+\eta_j^{[m]}\left(k+1\right)e_j^{[m]}\left(k+1\right)\hat{y}_j^{[m]}\left(k+1\right)\\
...
\begin{equation}
\text{ЯКЩО } x_i \text{ У } X_{li} \text{ ТОДІ ВИХІД  } w_{li},
\end{equation}
\medskip
...
\begin{equation}
\begin{aligned}
\text{ЯКЩО } x_i \text{ У } X_{li} \text{ ТО ВИХІД }\\
