...
Градієнтна оптимізація критерію \eqref{eq:RosenblattLearningCriterion} відносно $w_j^{[m]}$ можна записати у вигляді
...
Використовуючи momentum term \cite{ref74,ref75,ref76} (не змогла нормально перекласти українською) можна удосконалити процесс корегування синаптичних вагових коефіцієнтів під час навчаняя. Тоді, замість критерію навчання \eqref{eq:RosenblattLearningCriterion} слід використовувати функцію
...
\begin{cases}
\frac{x_i-c_{j,l-1,i}^{[1]}}{c_{jli}^{[1]}-c_{i,l-1,i}^{[1]}} \text { if } x_i\in\left[c_{j,l-1,i}^{[1]},c_{jli}^{[1]}\right],\\
...
При використанні критерію навчаняя з \hl{momentum term} \eqref{eq:RosenblattLearningCriterion} замість \eqref{eq:CompactLearningCriterion} отримаємо остаточний метод навчання нео-фаззі нейрона
...
що відрізняється від виразу \eqref{eq:NFNCascadeOutput} для звичайних $NFN$ тим, що містить у $p+1$ більше параметрів, що настроються.
...
Вихідні сигнали, сгенеровані нейронами пулу кожного з каскадів, можна об'єднати у окремому вузлі, нейроні $GN^{[m]}$, з точністю $\hat{y}^{*[m]}\left(k\right)$, не меншою від точності будь-якого нейрону пулу $\hat{y}_j^{[m]}\left(k\right)$. Це завдання можна вирішити за допомогою підходу ансамлей нейронних мереж.
...
Мінімуючи \eqref{eq:PoolOptimizatoinLaGrangeFunction} відносно $c^{[m]}$, отримуємо
